...
This is not a real vmap node, but most nodes have these properties so it is our best guess.
...
- children \<`element_array`\>\
    All the children of this node, they can be other nodes directly (when the node only exists once) or GUID references to other nodes 
